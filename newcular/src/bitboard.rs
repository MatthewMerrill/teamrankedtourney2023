...
#[inline]
fn east(x: u64) -> u64 {
    (x & 0b001111110111111011111101111110111111011111101111110111111) << 1
}

#[inline]
fn west(x: u64) -> u64 {
    (x & 0b011111101111110111111011111101111110111111011111101111110) >> 1
}
...
impl BitBoard {
    ...
    fn get_king_moves(&self, pos: u64) -> u64 {
        ...
        ret |= (west(nort) | east(nort)) & !self.player_one_mask;
        ret
    }
    ...
    fn get_knight_moves(&self, pos: u64) -> u64 {
        ...
        // Forward moves
        positions |= (pos & 0x007cf9f3e7cf9fu64) << (1 * 7 + 2);
        positions |= (pos & 0x01f3e7cf9f3e7cu64) << (1 * 7 - 2);
        positions |= (pos & 0x0001fbf7efdfbfu64) << (2 * 7 + 1);
        positions |= (pos & 0x0003f7efdfbf7eu64) << (2 * 7 - 1);
        positions &= !(self.piece_mask & self.player_one_mask);
...
mod test {
    ...
    fn opening_board_moves() {
        ...
        let mut expected = vec![
            "B1B1", "B1C3", "B1A3", "C1C1", "C1C2", "C1C3", "D1D1", "D1C2", "D1E2",
